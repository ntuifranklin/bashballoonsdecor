...
async function placeCategoriesTablesInCacheMiddleware (request, response, next) {
    if (isRedisWorking()) {
        ...
        if (cachedValue && typeof cachedValue != "undefined") {
            ...
            //read from database and place in cache
            var categories = await getCategories (tableName=CATEGORIES_TABLE) ;
            categoriesString = JSON.stringify(categories);
            console.log(`categories string read from database : ${categoriesString}`);
            //write categories to cache
            // this write option allows a longer categories table stored in memory
            const customized_write_option = 
            {
                
                EX: 43200, // 3600 is 1h , while 43200 is 12h
                NX: false, // write the data even if the key already exists
            } ;
            var categories = await getCategories (tableName=CATEGORIES_TABLE) ;
            await writeDataToRedisCache(key, categoriesString, customized_write_option);
            
            //request.locals.CATEGORIES_TABLE = categories ;
            // continue to the controller function
            next();
        }
    } else {
...
exports.placeCategoriesTablesInCacheMiddleware = placeCategoriesTablesInCacheMiddleware ;